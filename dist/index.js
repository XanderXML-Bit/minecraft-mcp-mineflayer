...
import { StateTransition, BotStateMachine, EntityFilters, BehaviorFollowEntity, BehaviorLookAtEntity, BehaviorGetClosestEntity, NestedStateMachine } from "mineflayer-statemachine";
const bots = new Map();
function getBotOrThrow(username) {
    if (username && bots.has(username))
        return bots.get(username);
    if (username && !bots.has(username))
        throw new Error(`Bot '${username}' not found`);
    const [first] = bots.values();
    if (!first)
        throw new Error("No active bots. Use joinGame first.");
    return first;
}
async function joinGame(params) {
    ...
    await new Promise((resolve, reject) => {
        ...
        const onKicked = (reason) => { cleanup(); reject(new Error(String(reason))); };
        const cleanup = () => {
...
async function leaveGame(params) {
...
async function goToKnownLocation(params) {
...
async function lookAt(params) {
    const bot = getBotOrThrow(String(params.username || ""));
    const x = Number(params.x), y = Number(params.y), z = Number(params.z);
    await bot.lookAt(new Vec3(x, y, z), true);
    return { ok: true };
}
async function jump(params) {
    const bot = getBotOrThrow(String(params.username || ""));
...
async function moveInDirection(params) {
    const bot = getBotOrThrow(String(params.username || ""));
...
async function readChat(params) {
    const bot = getBotOrThrow(String(params.username || ""));
...
async function attackSomeone(params) {
    const bot = getBotOrThrow(String(params.username || ""));
    const type = String(params.targetType || "player");
    const name = params.targetName ? String(params.targetName) : undefined;
    let entity;
    if (name) {
        entity = Object.values(bot.entities).find((e) => e.name === name || e.username === name);
    }
    else {
        entity = Object.values(bot.entities).find((e) => type === "player" ? e.type === "player" : e.kind === type);
    }
    if (!entity)
...
async function openInventory(params) {
    const bot = getBotOrThrow(String(params.username || ""));
...
async function dropItem(params) {
    const bot = getBotOrThrow(String(params.username || ""));
    ...
    for (const i of items) {
        if (count && i.count > count) {
            await bot.toss(i.type, null, count);
            break;
        }
...
async function giveItemToSomeone(params) {
    const bot = getBotOrThrow(String(params.username || ""));
...
async function lookAround(params) {
    const bot = getBotOrThrow(String(params.username || ""));
...
async function eatFood(params) {
    const bot = getBotOrThrow(String(params.username || ""));
    // @ts-ignore
    if (!bot.autoEat)
        throw new Error("auto-eat not available");
    // @ts-ignore
    await bot.autoEat.eat();
    return { ok: true };
}
async function sendToolCall(req) {
...
async function main() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => sendToolCall(req));
